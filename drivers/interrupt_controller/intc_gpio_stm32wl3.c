...
/**
 * @brief Driver for STM32WL3 GPIO interrupt controller
 *
 * In this file, "EXTI" should be understood as "GPIO interrupt controller".
 * There is no "External interrupt/event controller (EXTI)" in STM32WL3 MCUs.
 */

#define DT_DRV_COMPAT st_stm32wl3_gpio_intc
...
/* driver data */
struct stm32wl3_gpio_intc_data {
...
static struct gpio_irq_cb_wrp *irq_cb_wrp_for_line(stm32_gpio_irq_line_t line)
{
	...
	struct stm32wl3_gpio_intc_data *const data = dev->data;
...
static void stm32wl3_gpio_isr(const void *userdata)
{
	const struct wl3_gpio_isr_argblock *arg = userdata;
 ...
 */
static struct stm32wl3_gpio_intc_data gpio_intc_data;
...
#define INIT_INTC_PORT_INNER(node, pidx, plin)	\
	static const struct wl3_gpio_isr_argblock			\
		port ##pidx ##_argblock = {				\
	...
	IRQ_CONNECT(DT_IRQN_BY_IDX(node, pidx),				\
		...
		stm32wl3_gpio_isr, &port ##pidx ##_argblock, 0);	\
									\
	irq_enable(DT_IRQN_BY_IDX(node, pidx))

#define STM32WL3_INIT_INTC_FOR_PORT(_PORT)				\
 ...
 */
static int stm32wl3_gpio_intc_init(const struct device *dev)
{
	ARG_UNUSED(dev);

	STM32WL3_INIT_INTC_FOR_PORT(A);

	STM32WL3_INIT_INTC_FOR_PORT(B);

	return 0;
}

DEVICE_DT_DEFINE(INTC_NODE, &stm32wl3_gpio_intc_init,
...
/**
 * @internal
 * STM32WL3 GPIO interrupt controller driver:
...
void stm32_gpio_intc_select_line_trigger(stm32_gpio_irq_line_t line, uint32_t trg)
{
	...
	case STM32_GPIO_IRQ_TRIG_NONE:
		/**
		 * There is no NONE trigger on STM32WL3.
	...
	/* Since it is not possible to disable triggers on STM32WL3,
