...
#define DT_DRV_COMPAT st_stm32wl3_flash_controller
...
/* <soc.h> also brings "stm32wl3x_hal_flash.h"
 * and "system_stm32wl3x.h", which provide macros
...
LOG_MODULE_REGISTER(flash_stm32wl3x, CONFIG_FLASH_LOG_LEVEL);
...
{
	const struct flash_wl3x_data *data = dev->data;
 ...
 */
int flash_wl3x_read(const struct device *dev, off_t offset,
...
int flash_wl3x_write(const struct device *dev, off_t offset,
			const void *buffer, size_t len)
{
	struct flash_wl3x_data *data = dev->data;
	int res;
...
int flash_wl3x_erase(const struct device *dev, off_t offset, size_t size)
{
	struct flash_wl3x_data *data = dev->data;
	int res;
...
const struct flash_parameters *flash_wl3x_get_parameters(
...
static int flash_wl3x_get_size(const struct device *dev, uint64_t *size)
{
	ARG_UNUSED(dev);
...
void flash_wl3x_pages_layout(const struct device *dev,
				const struct flash_pages_layout **layout,
				size_t *layout_size)
{
	/**
	 * STM32WL3 flash: single bank, 2KiB pages
...
static DEVICE_API(flash, flash_wl3x_api) = {
	.erase = flash_wl3x_erase,
	.write = flash_wl3x_write,
	.read = flash_wl3x_read,
	.get_parameters = flash_wl3x_get_parameters,
	.get_size = flash_wl3x_get_size,
#ifdef CONFIG_FLASH_PAGE_LAYOUT
	.page_layout = flash_wl3x_pages_layout,
#endif
	/* extended operations not supported */
};

int stm32wl3x_flash_init(const struct device *dev)
{
	struct flash_wl3x_data *data = dev->data;
 ...
 */
static struct flash_wl3x_data wl3x_flash_drv_data;

DEVICE_DT_INST_DEFINE(0, stm32wl3x_flash_init, NULL,
		    &wl3x_flash_drv_data, NULL, POST_KERNEL,
		    CONFIG_FLASH_INIT_PRIORITY, &flash_wl3x_api);
