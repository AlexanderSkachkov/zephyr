...
#define DT_DRV_COMPAT st_stm32wl3_adc
...
LOG_MODULE_REGISTER(adc_stm32wl3, CONFIG_ADC_LOG_LEVEL);
...
/* The STM32WL3 has a 12-bit ADC, but the resolution can be
...
BUILD_ASSERT(STM32_HCLK_FREQUENCY >= (8 * 1000 * 1000),
	"STM32WL3: system clock frequency must be at least 8MHz to use ADC");

/**
 * Driver private structures
 */
struct adc_stm32wl3_data {
...
/**
 * In STM32CubeWL3 v1.0.0, the LL_GetPackageType is buggy and returns wrong values.
...
static inline struct adc_stm32wl3_data *drv_data_from_adc_ctx(struct adc_context *adc_ctx)
{
	return CONTAINER_OF(adc_ctx, struct adc_stm32wl3_data, ctx);
}
...
/* Tracks whether the ADC currently holds the PM policy locks, so the
 * helpers below are idempotent. The locks are meant to be held only while
 * a conversion is in flight — acquired in adc_exit_idle_mode() at read
 * start, released in adc_enter_idle_mode() at read completion. But
 * PM_DEVICE_ACTION_SUSPEND also routes through adc_enter_idle_mode() to
 * power the ADC down, and at suspend time the ADC is normally already idle
 * (an in-flight read holds the SUSPEND_TO_RAM lock, which would itself
 * block system suspend). Without this guard that path issues an unmatched
 * pm_policy_state_lock_put(): with CONFIG_ASSERT off the count underflows
 * below zero, and since the policy treats ANY non-zero count as "locked",
 * SUSPEND_TO_RAM (Deepstop) is wedged off for good after the first suspend
 * cycle. Single-instance driver, so a file-static flag is sufficient.
 */
static bool adc_pm_locked;

static void adc_acquire_pm_locks(void)
{
	unsigned int key = irq_lock();
	bool already = adc_pm_locked;
...
{
	uint16_t gain;
#if defined(CONFIG_SOC_STM32WL39XX) || defined(CONFIG_SOC_STM32WL35XX)
...
#endif /* CONFIG_SOC_STM32WL39XX | CONFIG_SOC_STM32WL35XX */
...
#if SMPS_MODE != STM32WL3_SMPS_MODE_OFF
	/* Disable SMPS synchronization */
	LL_ADC_SMPSSyncDisable(adc);
#endif /* SMPS_MODE != STM32WL3_SMPS_MODE_OFF */
 ...
 */
static void schedule_and_start_adc_sequence(ADC_TypeDef *adc, struct adc_stm32wl3_data *data)
{
...
#if defined(CONFIG_ADC_STM32_DMA)
	const struct adc_stm32wl3_config *config = data->dev->config;
	int err;
...
static inline void handle_end_of_sequence(ADC_TypeDef *adc, struct adc_stm32wl3_data *data)
{
...
{
	const struct adc_stm32wl3_config *config = dev->config;
	struct adc_stm32wl3_data *data = dev->data;
...
#if !defined(CONFIG_ADC_STM32_DMA)
void adc_stm32wl3_isr(const struct device *dev)
{
	const struct adc_stm32wl3_config *config = dev->config;
	struct adc_stm32wl3_data *data = dev->data;
...
static void adc_stm32wl3_dma_callback(const struct device *dma, void *user_data,
					uint32_t dma_channel, int dma_status)
{
	struct adc_stm32wl3_data *data = user_data;
	 ...
	 * but may be not applicable to STM32WL3 series' ADC.
	 */
...
{
	struct adc_stm32wl3_data *data = drv_data_from_adc_ctx(ctx);
	const struct adc_stm32wl3_config *config = data->dev->config;
 ...
 */
int adc_stm32wl3_channel_setup(const struct device *dev,
...
int adc_stm32wl3_read(const struct device *dev,
			const struct adc_sequence *sequence)
{
	CHECKIF(dev == NULL) { return -ENODEV; }
	struct adc_stm32wl3_data *data = dev->data;
...
int adc_stm32wl3_read_async(const struct device *dev,
...
static DEVICE_API(adc, api_stm32wl3_driver_api) = {
	.channel_setup = adc_stm32wl3_channel_setup,
	.read = adc_stm32wl3_read,
#if defined(CONFIG_ADC_ASYNC)
	.read_async = adc_stm32wl3_read_async,
#endif /* CONFIG_ADC_ASYNC */
	.ref_internal = 3600U	/* ADC_REF_INTERNAL is mapped to Vinput 3.6V range */
};

int adc_stm32wl3_init(const struct device *dev)
{
	const struct device *clk = DEVICE_DT_GET(STM32_CLOCK_CONTROL_NODE);
	const struct adc_stm32wl3_config *config = dev->config;
	struct adc_stm32wl3_data *data = dev->data;
	...
	IRQ_CONNECT(DT_IRQN(ADC_NODE), DT_IRQ(ADC_NODE, priority),
		adc_stm32wl3_isr, DEVICE_DT_GET(ADC_NODE), 0);
...
static int adc_stm32wl3_pm_action(const struct device *dev,
...
static const struct adc_stm32wl3_config adc_config = {
...
static struct adc_stm32wl3_data adc_data = {
	...
	.dmac_config = {
		...
		.block_count = 1,
		.dma_callback = adc_stm32wl3_dma_callback,
...
PM_DEVICE_DT_DEFINE(ADC_NODE, adc_stm32wl3_pm_action);

DEVICE_DT_DEFINE(ADC_NODE, adc_stm32wl3_init, PM_DEVICE_DT_GET(ADC_NODE),
	&adc_data, &adc_config, POST_KERNEL, CONFIG_ADC_INIT_PRIORITY,
	&api_stm32wl3_driver_api);
