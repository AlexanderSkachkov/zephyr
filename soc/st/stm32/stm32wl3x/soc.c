 ...
 *
 * On STM32WL3 series, after RESET, the system clock frequency is 16MHz.
 */
 ...
 * Data type definition comes from @ref system_stm32wl3xx.h
 */
extern RAM_VR_TypeDef RAM_VR;

/** Power Controller node (shorthand for upcoming macros) */
#define PWRC DT_INST(0, st_stm32wl3_pwr)
...
static void configure_smps(void)
{
	...
	if (SMPS_MODE == STM32WL3_SMPS_MODE_PRECHARGE) {
#if defined(PWR_CR5_SMPS_PRECH_CUR_SEL)
		/**
...
void soc_early_init_hook(void)
{
	...
	/* On reset, the 48MHz HSI is selected as input to
	 * the SYSCLKPRE prescaler, set to 3, resulting in
